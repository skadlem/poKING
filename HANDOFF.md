...
| 8 | ~~`rollout.py` — both seats recording~~ **DONE `x`-below**: ladder B's BR data arrives through `exploit.best_response(harvest=...)`, so `_collect_one` never needed the change. "Both seats" is instead NFSPStrategy per-seat step buffers (`_steps[seat]`) — one Pi instance seated twice records every decision; asserted rows == engine decision count on a mirror session | small |
| 9 | **DONE `x`-below**: `train_nfsp.py` — ladder-B loop (BR <- best_response(Pi, harvest); Pi <- fit on reservoir), heads-up by construction. Progress signal: the BR's training curve should fall as Pi improves. Smoke-verified 3 rounds; strength NOT yet claimed — that is step 10 | medium |
