...
| 10 | **DONE (failed honestly) — step 10 verdict, both campaigns measured.** Campaign #2 (fixed data path, 30 rounds x 40 iters x 2000 hands, ~36 min): fit loss held ~0.4 nats under the 1.712 coin-flip floor and the per-round BR curves FALL monotonically (+1696 -> ~0 by round 17 -> -27 at round 29) — the loop is now healthy and campaign #1's diagnosis was right. But `exploit --target nfsp` on pi_last: **737.5 ±86.0 (seed 7) and 1011.1 ±86.5 (seed 11), both converged** — vs the shipped PPO's 670.6 ±68.8 (seed 7). The success condition ("well below 670.6") failed on both seeds, and duplicate vs the heuristic says NFSP loses **-285.9 ±34.7** (PokerBot ahead). A control probe of the PPO at seed 11 printed 176.3, but that is NOT "670.6 was luck": the probe is a LOWER bound and the seed-11 exploiter's own curve ended at +143 (vs +495 for the seed-7 NFSP exploiter) — a weaker exploiter proves less. Same-seed comparison: 737.5 (NFSP) > 670.6 (PPO), both converged, so Pi is at best no better, plausibly worse. Verdict: ladder B at 30 rounds is not competitive with the shipped agent. Mechanism (measured): the exploiter opens at entropy 1.899 ~ log 9 and collapses it to 0.53 in 240k hands — the average policy is near-uniform. Open question: is Pi near-uniform BY DESIGN at 30 fictitious-play moves (the average of weak early BRs; its ceiling only rises with rounds) or underfit (loss 1.31 vs ~1.0-1.1 behaviour entropy)? Campaign #1 negative evidence: 1073.8 ±94.2 / 932.7 ±85.8. | — |
...
| `models/nfsp/` | campaign #2 pi_last.pt (round 29, the step-10 artifact) + per-10 checkpoints |
...
| `models/nfsp_full2.log`, `nfsp_probe2.log`, `nfsp_probe_s11.log`, `ppo_probe_s11.log`, `nfsp_dup.log` | the step-10 measurement trail |
