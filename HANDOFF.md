...
| 10 | **first measurement (pilot), probe converged:** `exploit --target nfsp` on the 5-round/20-iter pilot = **1011.5 ±94.2 bb/100**, training curve -126 -> +827 — a real bound, not a failed probe. It is WORSE than the shipped PPO's 670.6, which says nothing about NFSP and everything about the pilot scale: 100k hands harvested vs PPO's 1.2M. The claim to earn is still "well below 670.6"; that needs the full-scale run (30 rounds x 40 iters x 2000 hands ≈ 2.4M BR hands, ~1-2 h). Ladder-B loop health observed at pilot: round 0's BR ends +1349 vs untrained Pi; after ONE fit the same oracle trains to only +39..+240 — Pi got harder to exploit by ~5x, the curve direction the loop promises | — |
