...
- Produces: `Policy(rng: random.Random, risk_cfg: RiskConfig | None = None, mc_iters: int = 150)` with `decide(state, player_id, summary: OpponentSummary | None, detection: DetectionResult | None) -> Action`. Every returned `Action` has a non-empty `reason` and is legal per `state.legal_actions`. Behavior: value-bet when equity ≥ 0.6; bluff candidates for weak hands tied to fold frequency; softmax sampling over candidates (never fully deterministic); mirror mode (p_mirror > 0.6) uses larger bet fractions; risk cap applied to bet/raise amounts with fallback to call/check when capping makes the action illegal. `mc_iters` controls the Monte Carlo equity iteration count (default 150; tests lower it for speed).
...
- Produces: `PokerBot(rng=None, risk_cfg=None, num_players=6, mc_iters=150)` implementing `Strategy`: `decide(state, player_id) -> Action` (uses the first active opponent's summary/detection; sets `mirror_mode` when p_mirror ≥ 0.6), `on_hand_end(result, my_seat)` (updates models + detector), `mirror_mode: bool` attribute. `mc_iters` is passed through to `Policy` (tests lower it for speed).
...
> **Performance note (added after Task 15 hang):** `monte_carlo_equity` at 150 iterations costs ~17 ms per call; a self-play hand with 6 PokerBots does ~60-90 MC calls, making the original 400-hand symmetric test ~8 minutes and the whole bench suite ~10-15 min. Fix: `Policy`/`PokerBot` accept `mc_iters` (default 150, production unchanged); bench tests use `mc_iters=10` and reduced hand counts (100 deterministic, 150 self-play, 30 benchmark) so the bench suite runs in well under a minute. The 4σ symmetric bound still holds at these sizes.
