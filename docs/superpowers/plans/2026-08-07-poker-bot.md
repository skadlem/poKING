...
**Interfaces:**
- Produces: `Card(rank: int, suit: int)` with `__slots__`, `__repr__` (`"As"`), `__eq__`, `__hash__`; `all_cards() -> list[Card]`; `card_from_str(s: str) -> Card`; `Deck(rng: random.Random | None, cards: Sequence[Card] | None) -> None`, `Deck.draw(n: int = 1) -> list[Card]`, `Deck.remaining -> int`.
...
class Deck:
    ...
    def __init__(self, rng: random.Random | None = None, cards: Sequence[Card] | None = None) -> None:
...
def make_game(strategies, deck_cards=None, dealer=0, stacks=None):
    deck = Deck(random.Random(1), [card_from_str(s) for s in deck_cards]) if deck_cards else None
...
def make_game(strategies, deck_cards, stacks, dealer=0):
    deck = Deck(random.Random(1), [card_from_str(s) for s in deck_cards])
...
def test_side_pot_multiway():
    ...
    g = make_game(
        ...
        deck_cards=["As", "Ah", "Ks", "Kh", "Qs", "Qh", "2c", "3c", "4c", "5c", "6c"],
...
def test_split_pot_with_odd_chip():
    ...
    g = make_game(
        ...
        deck_cards=["As", "Ah", "Ad", "Ac", "Ks", "Kh", "2c", "3c", "4c", "5c", "6c"],
...
def test_robot_like_scores_high():
    ...
    assert BotDetector().detect(1, s).p_is_bot > 0.7
...
def test_mirror_same_actions():
    det = BotDetector()
    for _ in range(50):
        acts = [(0, "preflop", Action.raise_to(6, "r")), (1, "preflop", Action.call(4, "c"))]
...
def test_mirror_mode_increases_bet_size(monkeypatch):
    def mean_bet(mirror):
        ...
        monkeypatch.setattr("pokr.policy.monte_carlo_equity", lambda *a, **k: 0.85)
        bets = [p.decide(s, 0, None, det).amount for _ in range(200)
                if p.decide(s, 0, None, det).action_type == ActionType.BET]
...
def test_selfplay_is_symmetric():
    ...
    assert abs(r.total_bb) < 40  # no rake, symmetric matchup
